...
int main()
{
	...
	while(round)
	{
		...
		delay(1000);
		clear();
		printText("Play again?", 50, 80, RGBToWord(0xff, 0, 0), 0);
		if ((GPIOB->IDR & (1 << 11))==0)
		{
			round = 1;
		}
	...
	*player_score = 0;
	...
	while(game_mode)
	{
			...
			isInside(wolf2X, wolf2Y, 12, 16, x, y + 16) || isInside(wolf2X, wolf2Y, 12, 16, x + 12, y + 16))
		{
			...
			clear();
				printText("Game Over", 30, 60, RGBToWord(0xff, 0, 0), 0);    // Adjust position if needed
				printText("Hit by Wolf :(", 20, 80, RGBToWord(0xff, 0, 0), 0);  // Adjust position if needed
